...
export function ChessGame() {
  ...
  const [aiLastMove, setAiLastMove] = useState<{ from: Square; to: Square } | null>(null) // Track AI move separately
  ...
  const [pendingBlunderEval, setPendingBlunderEval] = useState<MoveEvaluation | null>(null)
  ...
  useEffect(() => {
    ...
    if (gameState.isCheckmate || gameState.isStalemate || gameState.isDraw) {
      ...
      const newStats = updateStatsAfterGame(playerStats, result, currentDifficulty)
      setPlayerStats(newStats)

      // Save to database if logged in
      if (user) {
        const scores = gameEvaluations.map((e) => {
          switch (e.type) {
            case "brilliant":
            case "excellent":
              return 1.0
        ...
        const variance =
          scores.length > 1 ? scores.reduce((sum, s) => sum + Math.pow(s - ams, 2), 0) / scores.length : 0
  ...
  const handleBlunderUndo = () => {
    if (historyIndex > 0) {
      ...
      setMoveNotations((prev) => prev.slice(0, -1))
